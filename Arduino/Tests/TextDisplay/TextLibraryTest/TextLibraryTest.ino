...
void setup() {
  ...
  Serial.println("ok");
  Serial.println(text.Alphabet[0].line1);
}

void loop(){
  
  /*
  char myWord[] = "PRINCESS WANDA";
        ...
        if(charArray[i] == ' '){
          ...
          for(int c = 0; c < 3; c++){
            if(((colPos + (2-c)) < 14) && ((colPos + (2-c)) >= 0)){
              if(bitRead(text.Alphabet[index].line1,c)) strip.setPixelColor(pgm_read_word(&(pixelMatrice[rowPos][colPos + (2-c)])), textColor);
              if(bitRead(text.Alphabet[index].line2,c)) strip.setPixelColor(pgm_read_word(&(pixelMatrice[rowPos + 1][colPos + (2-c)])), textColor);
              if(bitRead(text.Alphabet[index].line3,c)) strip.setPixelColor(pgm_read_word(&(pixelMatrice[rowPos + 2][colPos + (2-c)])), textColor);
              if(bitRead(text.Alphabet[index].line4,c)) strip.setPixelColor(pgm_read_word(&(pixelMatrice[rowPos + 3][colPos + (2-c)])), textColor);
              if(bitRead(text.Alphabet[index].line5,c)) strip.setPixelColor(pgm_read_word(&(pixelMatrice[rowPos + 4][colPos + (2-c)])), textColor);
            }
          }
          colPos += 4;
        }
